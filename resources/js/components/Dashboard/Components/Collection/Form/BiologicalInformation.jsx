...
function BiologicalInformation({ name, handleDelete, length, form }) {
    ...
    const handleWormsApi = async (info) => {
        ...
        const sources = await (
            await fetch(`${url}/AphiaSourcesByAphiaID/${info?.AphiaID}`)
        ).json();
        ...
        updateFields({
            authority: info.valid_authority,
            identification: info.scientificname,
    ...
    const handleSearch = async () => {
        ...
        const aphias = await (
            await fetch(`${url}/AphiaRecordsByName/${speciesName}`)
        ).json();
    ...
    const content = (
        <div>
            {wormsContent.map((element) => (
                <WormsOption
                    key={element.AphiaID}
                    onClick={() => handleWormsApi(element)}
                >
                    {element.rank}, {element.scientificname} (
                    {element.valid_authority})
                </WormsOption>
            ))}
        </div>
    );
    ...
    return (
        <>
            <Row type="flex" style={{ marginBottom: "20px" }}>
                <Subtitle>
                    Biological identification & samples information
                </Subtitle>
                {length > 1 && (
                    <Delete
            ...
            <Row type="flex" align="bottom" gutter={32}>
                <Col xs={24} md={8}>
                    <Form.Item
                        label="Highest taxonomic level*"
                        name={[name, "level"]}
                        rules={[
                            {
                                ...requiredRule,
                                message: "'taxonomic level' is required",
                            },
                        ]}
                    >
                ...
                <Col xs={24} md={8}>
                    <Popover
                        visible={wormsContent.length}
                        content={content}
                        title="Would you like to use existing information for any of the following species?"
                    >
                        <Form.Item
                        ...
                        >
                            <Input.Search
                                loading={loading}
                                enterButton
                                onSearch={handleIdentificationChange}
                                placeholder="Identification based on the highest taxonomic level"
                            />
                        </Form.Item>
                    </Popover>
                </Col>
                ...
                <Col xs={24} md={6}>
                    <Form.Item
                        label="Year of the first record (if NIS or cryptogenic)"
                        name={[name, "year_first_report"]}
                        rules={[{ required: false }]}
                    >
                ...
                <Col xs={24} md={6}>
                    <Form.Item
                        ...
                        rules={[
                            {
                                ...requiredRule,
                                message: "'maturity stages' are required",
                            },
                        ]}
                    >
